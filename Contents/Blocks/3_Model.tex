...
We consider a data trading game on a rooted tree $\calT$ that generalizes the chain-based formulation of \citet{ma2026incentivizing}.
...
\item \textbf{Budget Feasibility:} $\sum_{j \in \calI \setminus \calL} \pi(i,j) \le 1$ for all $i \in \calI \setminus \{r\}$. The revenue of ancestors can not exceed the payment from the buyer in any trade.
\end{enumerate}
\end{definition}
...
\]
\item \textup{(Bayes-consistency)} At every information set $(h_i, v_i)$ reached with positive probability under $(\tbmp^*, \tbmx^*)$, the belief $\tmu_i(\cdot \mid h_i, v_i)$ equals the conditional distribution of $\bmv_{-i}$ induced by the prior $\calF$ and the profile $(\tbmp^*, \tbmx^*)$ given $(h_i, v_i)$.
\end{enumerate}
\end{definition}
