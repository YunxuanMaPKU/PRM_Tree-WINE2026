...
\item \textbf{OPT}: a budget-balanced PRM found by a zeroth-order optimizer maximizing equilibrium welfare, run separately for each model.
\end{itemize}
...
Varying $2 \le d\le 8$ and $m\in\{1,2,3\}$ with results shown in \cref{fig:exp-vary}, the advantage of PRMs over RAW generally grows with both depth $d$ and branching $m$---deeper, bushier trees create more downstream value to internalize---and is robust to violations of positivity and homogeneity. The growth with branching $m$ is a tree-specific phenomenon absent from the chain model \citep{ma2026incentivizing}, which our formulation is the first to exhibit.
