...
Since $\ts^{1,*}_j = \ts^{2,*}_j$ (proven in Part~(1)), it suffices to show $\tp^{2,*}_j(v_i) \le \tp^{1,*}_j(v_i)$ for all $v_i$, because then:
\[
E^1_j = \{\bmv : \ts^*_j(v_j) \ge \tp^{1,*}_j(v_i)\} \subseteq \{\bmv : \ts^*_j(v_j) \ge \tp^{2,*}_j(v_i)\} = E^2_j.
\]
...
\emph{Property 1.} If $\pi^1(j,i) = 0$, then condition~(b) forces $\pi^2(j,i) = 0$, so $r^1_i \equiv r^2_i \equiv 0$, $\Delta r_i \equiv 0$, and the implication~\eqref{eq:prf:partial:prop1} below holds trivially; assume henceforth $\pi^1(j,i) > 0$. Since $\ts^{1,*}_j = \ts^{2,*}_j$, we have $r^2_i(v_i, p) = \frac{\pi^2(j,i)}{\pi^1(j,i)}\, r^1_i(v_i, p)$ with coefficient $\frac{\pi^2(j,i)}{\pi^1(j,i)} \le 1$ by condition~(b). Consequently, for any $p, p'$:
\begin{equation}
\label{eq:prf:partial:prop1}
r^2_i(v_i, p) \ge r^2_i(v_i, p') \quad\Longrightarrow\quad \Delta r_i(v_i, p) \le \Delta r_i(v_i, p'),
\end{equation}
where $\Delta r_i \coloneqq r^2_i - r^1_i$, because $\Delta r_i = (\frac{\pi^2(j,i)}{\pi^1(j,i)} - 1) r^1_i$ and the coefficient is nonpositive.

\emph{Property 2.} For $p' > p$, we have $\one\{\ts^*_j(v_j) \ge p'\} \le \one\{\ts^*_j(v_j) \ge p\}$ pointwise. Since prices are nonnegative and $y_{m|j} \in \{0,1\}$, the integrand in $d^k_i$ is nonnegative and is multiplied by $\one\{\ts^*_j(v_j) \ge p\}$, so $d^k_i(v_i, p)$ is weakly decreasing in $p$. For $\Delta d_i(v_i, p) \coloneqq d^2_i(v_i, p) - d^1_i(v_i, p)$:
\begin{align*}
\Delta d_i(v_i, p) &= \bbE\bigg[\one\{\ts^*_j(v_j) \ge p\} \cdot \sum_{m \in \Desc(j)} \big(\pi^2(m,i) - \pi^1(m,i)\big) \cdot p_m \cdot y_{m|j} \;\bigg|\; v_i\bigg].
\end{align*}
By condition~(c), $\pi^2(m,i) - \pi^1(m,i) \ge 0$ for all $m \in \Desc(j)$, so the summand is nonnegative; hence $\Delta d_i(v_i, p)$ is also weakly decreasing in $p$.

Now suppose for contradiction that $p^2 > p^1$. By optimality of $p^2$ under $\pi^2$:
\[
G^{(j),2}_i(v_i, p^2) \ge G^{(j),2}_i(v_i, p^1).
\]
Since $d^2_i$ is weakly decreasing in $p$ (Property~2) and $p^2 > p^1$, we have $d^2_i(v_i, p^2) \le d^2_i(v_i, p^1)$. Combined with the optimality inequality:
\begin{equation}
\label{eq:prf:partial:rstep}
r^2_i(v_i, p^2) \ge r^2_i(v_i, p^1) \quad\Longrightarrow\quad \Delta r_i(v_i, p^2) \le \Delta r_i(v_i, p^1),
\end{equation}
where the implication follows from Property~1. Define the quantity:
\begin{align*}
S \;&\coloneqq\; \big[G^{(j),1}_i(v_i, p^2) - G^{(j),1}_i(v_i, p^1)\big] - \big[G^{(j),2}_i(v_i, p^2) - G^{(j),2}_i(v_i, p^1)\big] \\
&=\; \big[\Delta r_i(v_i, p^1) - \Delta r_i(v_i, p^2)\big] + \big[\Delta d_i(v_i, p^1) - \Delta d_i(v_i, p^2)\big] \;\ge\; 0,
\end{align*}
where the first bracket is $\ge 0$ by~\eqref{eq:prf:partial:rstep} and the second bracket is $\ge 0$ by Property~2 (since $\Delta d_i$ is weakly decreasing and $p^2 > p^1$). Since $G^{(j),2}_i(v_i, p^2) \ge G^{(j),2}_i(v_i, p^1)$ and $S \ge 0$:
\[
G^{(j),1}_i(v_i, p^2) \ge G^{(j),1}_i(v_i, p^1).
\]
By optimality of $p^1$ under $\pi^1$, equality must hold: $G^{(j),1}_i(v_i, p^2) = G^{(j),1}_i(v_i, p^1)$. Then $S \ge 0$ forces $G^{(j),2}_i(v_i, p^2) = G^{(j),2}_i(v_i, p^1)$, so $p^1$ is also a maximizer of $G^{(j),2}_i(v_i, \cdot)$. But $p^1 < p^2$, and the smallest-maximizer tie-break selects the smallest maximizer under $\pi^2$; hence $p^2 = \tp^{2,*}_j(v_i) \le p^1$, contradicting $p^2 > p^1$. Therefore $\tp^{2,*}_j(v_i) \le \tp^{1,*}_j(v_i)$.
\qed
\end{proof}
