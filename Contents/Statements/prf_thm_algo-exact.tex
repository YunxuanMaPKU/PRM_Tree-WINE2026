\begin{proof}
We prove that \cref{alg:exact,alg:forward-dp} correctly compute a perfect Bayesian equilibrium in $O(N d\, S^4)$ time, where $d$ is the depth of $\calT$.
...
\[
\sum_{j \in \calI \setminus \{r\}} O(|\Subtree(j)| \cdot S^4) \;=\; O\bigg(\sum_{j \in \calI \setminus \{r\}} |\Subtree(j)|\bigg) \cdot S^4 \;=\; O(N d\, S^4),
\]
